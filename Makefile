...
destroy: stop
	docker volume rm -f ${BUILD_NAME}_netbox-device-plugin

bash:
	docker compose -f ${COMPOSE_FILE} -p ${BUILD_NAME} exec netbox /bin/bash

createsuperuser:
	docker compose -f ${COMPOSE_FILE} -p ${BUILD_NAME} exec netbox python manage.py createsuperuser

changepassword:
	docker compose -f ${COMPOSE_FILE} -p ${BUILD_NAME} exec netbox python manage.py changepassword admin
...
test:
	docker compose -f ${COMPOSE_FILE} -p ${BUILD_NAME} run ${COMMON_PARAMS} netbox python manage.py \
		test --keepdb /opt/netbox_device_view
